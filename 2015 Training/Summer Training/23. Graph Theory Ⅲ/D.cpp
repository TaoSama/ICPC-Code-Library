...
//  freopen("out.txt","w",stdout);
    ...
    while(T--) {
        ...
        int s = 0, t = xs.size() + 1;
        add_double(s, 1, 0, k); add_double(xs.size(), t, 0, k);
        for(int i = 1; i < xs.size(); ++i) {
            add_double(i, i + 1, 0, k);
        }
        for(int i = 1; i <= n; ++i) {
            x[i] = lower_bound(xs.begin(), xs.end(), x[i]) - xs.begin() + 1;
            y[i] = lower_bound(xs.begin(), xs.end(), y[i]) - xs.begin() + 1;
            add_double(x[i], y[i], -w[i], 1);
        }
        printf("%d\n", -minCostMaxFlow(s, t));
    }
    return 0;
