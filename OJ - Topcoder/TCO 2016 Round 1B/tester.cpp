...
char runTestCase(int caseNo, input in, output exp, int n, double timeOut, string & reportLine, bool silentMode) {
    ...
    if (testThread) {
        ...
        res = output( in.run(instance) );
        elapsed = (double)(clock() - s) / CLOCKS_PER_SEC;
        delete instance;

        correct = exp.u || tc_eq(exp.value, res.value);
    ...
    if (! silentMode) {
        cout << TEST_COLOR << "Test " << caseNo << COLOR_RESET <<": " << tm << endl;
        if (! correct) {
            cout << "Expected: " << pretty(exp.value) << endl;
        }
        cout << "Received: " << pretty(res.value) << endl;
        cout << "Time: "<< f02(elapsed)<<" s." << endl;
        cout << BAR_COLOR << string(BAR_LENGTH,'=') << COLOR_RESET << endl;
    }
